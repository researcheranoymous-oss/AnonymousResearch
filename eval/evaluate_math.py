...
# Benchmark registry: name -> (hub_id, split, trust_remote_code).
BENCHMARKS = {
...
# Root dir for curl-downloaded local benchmark parquet (override with env EVAL_DATASET_ROOT).
DATASET_LOCAL_ROOT = os.environ.get("EVAL_DATASET_ROOT", "/root/autodl-tmp/datasets")


def load_benchmark_dataset(dataset_name: str):
    """Load an eval benchmark, preferring a curl-downloaded local parquet copy.

    In environments that cannot reach the HF hub/mirror API (load_dataset fails with
    LocalEntryNotFoundError), download the parquet with curl into
    ``<EVAL_DATASET_ROOT>/<name>/`` and this reads it via pandas — which also bypasses the
    ``trust_remote_code`` loader scripts (aime25/hmmt25) and any embedded 'List' feature metadata
    that an older ``datasets`` rejects. Falls back to the hub when no local copy is present.
    """
    key = dataset_name.lower()
    if key not in BENCHMARKS:
        raise ValueError(f"Unknown dataset: {dataset_name}. Choose one of {sorted(BENCHMARKS)}")
    hub_id, split, trust = BENCHMARKS[key]
    local_dir = os.path.join(DATASET_LOCAL_ROOT, key)
    parquets = (
        sorted(glob.glob(os.path.join(local_dir, "**", f"*{split}*.parquet"), recursive=True))
        or sorted(glob.glob(os.path.join(local_dir, "**", "*.parquet"), recursive=True))
    )
    if parquets:
        print(f"Loading {dataset_name} from local parquet ({len(parquets)} file(s)) under {local_dir}")
        df = pd.concat([pd.read_parquet(p) for p in parquets], ignore_index=True)
        print(f"  local columns: {list(df.columns)}, rows: {len(df)}")
        return Dataset.from_pandas(df, preserve_index=False)
    print(f"Local copy not found under {local_dir}; loading {hub_id} from hub (needs network).")
    return load_dataset(hub_id, split=split, trust_remote_code=trust)
...
def evaluate_math500(
    ...
    print(f"Loaded {dataset_name} dataset with {len(dataset)} problems")
    ...
    all_prompts = []
    # `enable_thinking` is a Qwen3-specific chat-template kwarg; other families
    # (e.g. OLMo3) select thinking mode via checkpoint choice instead and don't
    # accept this kwarg at all.
    template_kwargs = {"enable_thinking": enable_thinking} if supports_enable_thinking else {}
...
def main():
    ...
    parser.add_argument(
        ...
        dest="supports_enable_thinking",
        action="store_false",
        help="Set for model families without a template `enable_thinking` kwarg (e.g. OLMo3); "
        "thinking mode is then controlled entirely by which checkpoint --base_model points at.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    if args.top_p is None:
        if args.supports_enable_thinking:
            ...
            print(
                ...
                "(Qwen3-tuned default; pass --top_p explicitly to override)"
            )
        else:
            raise ValueError(
                "--top_p must be set explicitly when --no_enable_thinking_kwarg is used: the auto-tuned "
                "default above is Qwen3-specific and would silently misconfigure other model families."
            )
    ...
    average_at_n_pct, results = evaluate_math500(
        ...
        enable_thinking=args.enable_thinking,
        supports_enable_thinking=args.supports_enable_thinking,
        val_n=args.val_n,
    )
