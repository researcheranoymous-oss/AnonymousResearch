...
from adaptive_masking import AdaptiveMaskingStore
...
class MASSTrainer(SFTTrainer):
    ...
    def __init__(
        ...
        use_reference_masking: bool = True,
        reference_mask_probability: float = 0.3,
        num_masked_teacher_views: int = 8,
        supports_enable_thinking: bool = True,
        anchor_coefficient: float = 0.5,
        weight_regularization: float = 1e-6,
        shapley_regularization: float = 0.1,
        ema_smoothing: float = 0.9,
        mask_probability_lower_bound: float = 0.03,
        mask_adaptation_rate: float = 0.5,
        enable_adaptive_masking: bool = False,
    ):
        if use_reference_masking and reason_first:
            raise ValueError("Masked-reference training does not support reason_first=True.")
        if use_reference_masking and not 0 < reference_mask_probability < 1:
            raise ValueError("reference_mask_probability must be strictly between 0 and 1.")
        if use_reference_masking and num_masked_teacher_views < 2:
        ...
        # Rank-local per-example adaptive-masking state (Sec. 2.3). Constructed before the
        # collator so the collator can read each example's current per-span q vector.
        # NOTE: enable_adaptive_masking defaults to False in this release — the Shapley
        # regression / masking-probability update internals are withheld pending paper
        # acceptance (see adaptive_masking.py). With the default False, masking stays
        # plain uniform Bernoulli(rho) per span, reproducing the paper's disclosed
        # "Weighting only" ablation (Table 3), not the "Full MASS" row.
        self.mask_state_store = (
            AdaptiveMaskingStore(
                rho=reference_mask_probability,
                mask_probability_lower_bound=mask_probability_lower_bound,
                mask_adaptation_rate=mask_adaptation_rate,
                ema_smoothing=ema_smoothing,
                shapley_regularization=shapley_regularization,
                adaptive=enable_adaptive_masking,
            )
            if use_reference_masking
            else None
        )
        ...
        if data_collator is None:
            data_collator = SelfDistillationDataCollator(
                ...
                use_reference_masking=use_reference_masking,
                reference_mask_probability=reference_mask_probability,
                num_masked_teacher_views=num_masked_teacher_views,
                mask_state_store=self.mask_state_store,
        ...
        self.shapley_regularization = shapley_regularization
        self.ema_smoothing = ema_smoothing
        self.mask_probability_lower_bound = mask_probability_lower_bound
        self.mask_adaptation_rate = mask_adaptation_rate
        self.enable_adaptive_masking = enable_adaptive_masking
    ...
    def generalized_jsd_loss(
        ...
        Args:
            ...
            teacher_logits:
                Tensor of shape (batch_size, sequence_length, vocab_size)
            teacher_probs:
                Optional averaged teacher probability distribution. When provided, it has
                already been temperature-scaled and takes precedence over teacher_logits.
            labels:
        ...
        if teacher_probs is not None:
            ...
            if top_k is not None and top_k > 0:
                _, top_k_indices = torch.topk(teacher_probs, k=top_k, dim=-1)
                student_logits = torch.gather(student_logits, dim=-1, index=top_k_indices)
                teacher_probs = torch.gather(teacher_probs, dim=-1, index=top_k_indices)
    ...
    @staticmethod
    def selective_weight(S_hat, V_hat, B_hat, lmbda, epsilon):
        """Bias-corrected selective weight (Eq. 5): the fraction of the empirical
        mask-marginal correction to retain, given its magnitude ``S_hat``, its
        finite-K sampling uncertainty ``V_hat``, and its alignment ``B_hat`` with
        the full-reference anchor. Clipped to [0, 1] since it is a shrinkage
        factor, not an amplifier.
        """
        numerator = S_hat.clamp(min=0) + lmbda * B_hat.clamp(min=0)
        denominator = S_hat.clamp(min=0) + V_hat + epsilon
        return (numerator / denominator).clamp(min=0.0, max=1.0)

    @staticmethod
    def weighted_forward_kl_loss(student_logits, teacher_probs, weights, labels, temperature):
        """Selective forward-KL distillation loss (Eq. 6):

            L = mean_t sg(w_t) * KL( sg(p_hat_t) || p_{S,t} )

        Both ``weights`` and ``teacher_probs`` are stop-gradient; only
        ``student_logits`` carries a gradient. This is exactly the ``beta=0``
        branch of ``generalized_jsd_loss`` with an extra per-token multiplier.
        """
        student_log_probs = F.log_softmax(student_logits / temperature, dim=-1)
        teacher_probs = teacher_probs.clamp_min(1e-8).detach()
        teacher_log_probs = torch.log(teacher_probs)
        # KL(teacher || student), matching F.kl_div's (input, target) convention with log_target=True.
        per_token_kl = F.kl_div(
            student_log_probs, teacher_log_probs, reduction="none", log_target=True
        ).sum(dim=-1)

        mask = labels != -100
        weighted = per_token_kl[mask] * weights.detach()[mask]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_ema_teacher:
            make_adapter_context = lambda: self._ema_teacher_context(model)
        elif self.fixed_teacher and is_peft_model(model):
            make_adapter_context = lambda: self.accelerator.unwrap_model(model).disable_adapter()
        else:
            make_adapter_context = nullcontext
        adapter_context = make_adapter_context()

        teacher_input_ids = inputs["teacher_input_ids"]
        teacher_attention_mask = inputs["teacher_attention_mask"]
        if teacher_input_ids.ndim != 3:
            raise ValueError("teacher_input_ids must have shape [num_views, batch, sequence].")

        anchor_input_ids = inputs.get("anchor_prompts")
        anchor_attention_mask = inputs.get("anchor_prompt_attention_mask")
        anchor_prompt_len = inputs.get("anchor_prompt_length")
        has_anchor = anchor_input_ids is not None

        with torch.no_grad(), adapter_context:
            teacher_probs_mean = None
            sum_sq_norms = None
            for view_index in range(teacher_input_ids.shape[0]):
                outputs_teacher = model(
                    input_ids=teacher_input_ids[view_index],
                    attention_mask=teacher_attention_mask[view_index],
                )
                teacher_logits = outputs_teacher.logits[:, teacher_prompt_len - 1 : -1, :]
                teacher_probs = F.softmax(teacher_logits / self.temperature, dim=-1)
                del outputs_teacher, teacher_logits

                if self.use_thinking_machines_loss:
                    view_prediction = torch.gather(
                        teacher_probs, dim=-1, index=sampled_token_ids.unsqueeze(-1)
                    ).squeeze(-1)
                else:
                    view_prediction = teacher_probs

                if teacher_probs_mean is None:
                    teacher_probs_mean = view_prediction
                else:
                    teacher_probs_mean.mul_(view_index / (view_index + 1)).add_(
                        view_prediction, alpha=1 / (view_index + 1)
                    )

                if has_anchor and not self.use_thinking_machines_loss:
                    # Single-pass variance accumulator (Eq. 4): sum_k||p_k-p_hat||^2
                    # = sum_k||p_k||^2 - K||p_hat||^2, so we only need the running
                    # sum of squared norms here, not the individual views.
                    view_sq_norm = teacher_probs.pow(2).sum(dim=-1)
                    sum_sq_norms = view_sq_norm if sum_sq_norms is None else sum_sq_norms + view_sq_norm

                del teacher_probs
                if view_prediction is not teacher_probs_mean:
                    del view_prediction
                empty_cache()

            anchor_probs = None
            if has_anchor and not self.use_thinking_machines_loss:
                outputs_anchor = model(input_ids=anchor_input_ids, attention_mask=anchor_attention_mask)
                anchor_logits = outputs_anchor.logits[:, anchor_prompt_len - 1 : -1, :]
                anchor_probs = F.softmax(anchor_logits / self.temperature, dim=-1)
                del outputs_anchor, anchor_logits

        if self.use_thinking_machines_loss:
            teacher_log_probs_sampled = torch.log(teacher_probs_mean.clamp_min(1e-8))
        else:
            teacher_probs_for_loss = teacher_probs_mean
        ...
        if self.use_thinking_machines_loss:
            ...
            # === Selective weighting (Eq. 3-6): bias-corrected S_hat, V_hat, B_hat from
            # the final mask-marginal p_hat, the anchor p_0, and the student's own p_S. None
            # of these need the individual per-view teacher probabilities, only the pooled mean.
            p_hat = teacher_probs_for_loss
            p_S = F.softmax(student_logits_for_loss / self.temperature, dim=-1).detach()
            num_views = teacher_input_ids.shape[0]

            V_hat = (sum_sq_norms - num_views * p_hat.pow(2).sum(dim=-1)) / (num_views * (num_views - 1))
            correction = p_S - p_hat  # \hat{g}_t
            anchor_delta = anchor_probs - p_hat  # p_0 - p_hat
            S_hat = (correction.pow(2).sum(dim=-1) - V_hat).clamp(min=0)
            B_hat = (correction * anchor_delta).sum(dim=-1) - V_hat

            weight = self.selective_weight(S_hat, V_hat, B_hat, self.anchor_coefficient, self.weight_regularization)

            loss = self.weighted_forward_kl_loss(
            ...
            # === Adaptive masking (Eq. 7-10): recompute each masked view's teacher
            # probabilities a second time (one at a time, so memory stays flat in K)
            # now that the final correction direction sg(p_S - p_hat) is known, to
            # estimate each example's per-span coalition utility R^(k) = v(A_k) and
            # update its per-span masking probabilities for the next visit.
            view_masks = inputs.get("view_masks")
            example_ids = inputs.get("example_ids")
            if self.mask_state_store is not None and view_masks is not None:
                correction_detached = correction.detach()
                anchor_detached = anchor_probs.detach()
                label_mask = shifted_labels != -100
                token_counts = label_mask.sum(dim=-1).clamp(min=1)

                dot_anchor = (correction_detached * anchor_detached).sum(dim=-1)
                utilities_by_view = []
                with torch.no_grad(), make_adapter_context():
                    for view_index in range(num_views):
                        outputs_view = model(
                            input_ids=teacher_input_ids[view_index],
                            attention_mask=teacher_attention_mask[view_index],
                        )
                        view_logits = outputs_view.logits[:, teacher_prompt_len - 1 : -1, :]
                        view_probs = F.softmax(view_logits / self.temperature, dim=-1)
                        del outputs_view, view_logits

                        dot_view = (correction_detached * view_probs).sum(dim=-1)
                        per_token_utility = (dot_anchor - dot_view) * label_mask
                        utilities_by_view.append(per_token_utility.sum(dim=-1) / token_counts)
                        del view_probs, dot_view, per_token_utility
                        empty_cache()

                utilities = torch.stack(utilities_by_view, dim=0)  # [K, batch]
                for example_index, example_id in enumerate(example_ids):
                    if example_id is None or example_id < 0:
                        continue
                    masks = view_masks[example_index]
                    if masks.shape[1] < 2:
                        continue  # A single-span reference has nothing to attribute.
                    self.mask_state_store.update(
                        example_id, masks, utilities[:, example_index].to(dtype=torch.float64, device="cpu")
                    )

            del student_logits_for_loss, teacher_probs_for_loss, p_hat, p_S, correction, anchor_delta
        else:
            # Temperature is applied inside generalized_jsd_loss
            loss = self.generalized_jsd_loss(
                student_logits=student_logits_for_loss,
                teacher_logits=None,
                teacher_probs=teacher_probs_for_loss,
                labels=shifted_labels,
                beta=self.beta,
            ...
            del student_logits_for_loss, teacher_probs_for_loss
    ...
    def training_step(
        ...
        if self.reason_first:
            ...
            with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                ...
                inputs["teacher_prompts"] = teacher_prompts_with_reasoning.unsqueeze(0)
                ...
                inputs["teacher_prompt_attention_mask"] = teacher_attention_mask.unsqueeze(0)
        ...
        num_teacher_views = teacher_prompts.shape[0]
        generation_ids_by_view = generation_ids.unsqueeze(0).expand(num_teacher_views, -1, -1)
        teacher_full_ids = torch.cat([teacher_prompts, generation_ids_by_view], dim=2)

        generation_attention_mask = generated_attention_mask[:, student_prompt_len:]
        generation_attention_mask_by_view = generation_attention_mask.unsqueeze(0).expand(
            num_teacher_views, -1, -1
        )
        teacher_attention_mask = torch.cat(
            [inputs["teacher_prompt_attention_mask"], generation_attention_mask_by_view], dim=2
        )
