#!/bin/bash
# Non-thinking OLMo3-7B ablation: points at the base (non-Think) OLMo3-7B checkpoint
# instead of toggling a template kwarg, since OLMo3 has no `enable_thinking` flag.
...
accelerate launch \
    ...
    mass_train.py \
    --model_name_or_path /root/autodl-tmp/models/OLMo-3-7B \
    --learning_rate 5e-6 \
    --max_grad_norm 0.1 \
    ...
    --run_config mass_olmo37b_gen1024_both_nonthink_fixteacher_temp11_forwardbeta0_clip1e-7 \
    --num_train_epochs 30 \
